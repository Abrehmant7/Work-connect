...
def evaluation_function(state,board,sgn):
    free = state
    if(len(free)):
        return None
    else:
        if sgn == "X":  # are we looking for X?
            value = 1   # yes - it's computer's side
        elif sgn == "O": # ... or for O?
            value = -1  # yes - it's our side
        else:
            value = None    # we should not fall here!
...
def actions(state, board):
    ...
    while len(free):
        ...
        if human_turn:
            draw_move(boardCopy)
            possible_states.append(free)  # ← Fix indentation here
			victor = evaluation_function(free, boardCopy, 'O')
        else:	
            draw_move(boardCopy)
            possible_states.append(free)
			victor = evaluation_function(free, boardCopy, 'O')
        if victor != None:
            break
        human_turn = not human_turn		
        free = make_list_of_free_fields(boardCopy)
...
while len(free):
	...
	if human_turn:
		enter_move(board)
		victor = evaluation_function(board,'O')
	else:	
		draw_move(board)
		victor = evaluation_function(board,'X')
	if victor != None:
		break
	human_turn = not human_turn		
	free = make_list_of_free_fields(board)
